...
| 로컬 토큰 삭제 후에도 바로 재로그인됨 | Cognito Hosted UI 쿠키와 소셜 IdP 세션. `Cognito 세션 로그아웃`으로 `/logout`을 호출하거나 브라우저 쿠키를 삭제 |
...
- 소셜 로그인 재테스트는 로컬 토큰 삭제만으로 충분하지 않다. Cognito Hosted UI `/logout` 호출 후 필요하면 Kakao 계정 세션도 로그아웃한다.
