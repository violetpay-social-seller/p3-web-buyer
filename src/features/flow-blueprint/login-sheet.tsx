...
import { startHostedUiLogin } from "@/shared/auth/cognito";
...
export function FlowLoginSheet() {
  ...
  return (
    <div className="flex h-full items-end bg-[var(--figma-color-surface-scrim)]">
      <section className="flex w-full flex-col items-center gap-[var(--figma-space-section-mobile)] rounded-t-[var(--figma-radius-lg)] bg-white px-[var(--figma-space-md)] py-[var(--figma-space-lg)] shadow-[var(--figma-shadow-dropdown)]">
        <div className="flex w-full justify-end">
          <Link aria-label="닫기" className="grid h-12 w-12 place-items-center" href="/">
            <IconAsset name="cancel.svg" />
          </Link>
        </div>
        <div className="text-center">
          <p className="text-label-md text-[var(--figma-color-text-secondary)]">3초면 주문가능!</p>
          <h1 className="mt-[var(--figma-space-sm)] text-display-lg">
            간편하게 로그인 후
            <br />
            주문이 가능해요!
          </h1>
        ...
        <div className="w-full">
          <button className="relative flex h-[52px] w-full items-center justify-center rounded-[var(--figma-radius-sm)] bg-[#ffe600] text-[16px] font-medium tracking-[-0.01em] text-black" onClick={handleLogin} type="button">
            <span className="absolute left-[18px] grid h-5 w-[22px] place-items-center">
              <IconAsset name="kakao-symbol.svg" size={22} />
            </span>
            카카오 로그인
          </button>
          <button className="relative mt-[var(--figma-space-sm)] flex h-[52px] w-full items-center justify-center rounded-[var(--figma-radius-sm)] border border-[#ccccd1] bg-white text-[16px] font-medium tracking-[-0.01em] text-black" onClick={handleLogin} type="button">
